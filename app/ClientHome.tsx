    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans selection:bg-blue-200 selection:text-blue-900 overflow-hidden relative">
        ...
        <div className="w-full max-w-[1600px] mx-auto px-4 md:px-8 flex flex-col lg:flex-row gap-4 md:gap-8 h-full items-stretch">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full">
              <h2 className="text-2xl md:text-3xl font-black text-white mb-6 flex items-center gap-3 tracking-tight shrink-0"><Bell className="text-blue-500" /> Latest Notices</h2>
              ...
              <div className="mt-6 pt-6 border-t border-slate-800 shrink-0">
                <a href="#apply" className="bg-blue-600 text-white w-full py-4 rounded-xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-500 transition-colors text-center flex items-center justify-center gap-2">Apply for Admission <ArrowRight className="h-5 w-5"/></a>
              </div>
            </div>
          </motion.div>
        </div>
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} className="mt-4 md:mt-8 mb-4 md:mb-0 bg-slate-800 p-2.5 md:p-3 rounded-full shadow-sm border border-slate-700 text-blue-400 z-10"><ChevronDown className="h-5 w-5 md:h-6 md:w-6" /></motion.div>
      </section>
          ...
          <motion.div className="flex whitespace-nowrap text-white font-black text-xs tracking-widest uppercase items-center" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 35, ease: "linear", repeat: Infinity }}>
            {[...Array(8)].map((_, i) => (
              <span key={i} className="flex items-center">
                ...
                <span className="bg-white text-blue-700 px-3 py-1 rounded-full text-[10px]">Click Now <ArrowRight className="h-3 w-3" /></span>
      ...
      <section id="toppers" className="relative pt-8 pb-10 md:py-24 overflow-hidden bg-white flex flex-col justify-center min-h-fit md:min-h-[85vh] z-20">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 w-full mb-8 md:mb-12">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 max-w-5xl mx-auto">
            <motion.div variants={fadeInUp} whileHover={{ y: -10 }} className="relative bg-white rounded-3xl p-1.5 border border-slate-200 shadow-xl hover:shadow-2xl transition-all duration-500 group overflow-hidden">
                  ...
                  {isMounted && (
                    <div className="absolute inset-0 flex items-center justify-center z-0">
                      {Array.from({ length: 12 }).map((_, i) => (
                          <motion.div key={`b1-${i}`} className="absolute w-1.5 h-1.5 bg-amber-400 rounded-full shadow-[0_0_6px_rgba(251,191,36,0.8)]" animate={{ x: [0, Math.cos((i*30)*(Math.PI/180)) * 160], y: [0, Math.sin((i*30)*(Math.PI/180)) * 160], scale: [0, 1.5, 0], opacity: [0, 1, 0] }} transition={{ duration: 1.2, repeat: Infinity, repeatDelay: 1.5 }} />
                      ))}
                    </div>
                  )}
              ...
              <div className="bg-slate-50 rounded-[1.3rem] h-full flex flex-col items-center p-6 md:p-8 relative z-10">
                ...
                <div className="relative mt-6 md:mt-8 mb-4 md:mb-6">
                  {isMounted && (
                    <div className="absolute inset-0 flex items-center justify-center z-0">
                      {Array.from({ length: 12 }).map((_, i) => (
                          <motion.div key={`c1-${i}`} className="absolute w-1.5 h-1.5 bg-blue-400 rounded-full shadow-[0_0_6px_rgba(96,165,250,0.8)]" animate={{ x: [0, Math.cos((i*30)*(Math.PI/180)) * 160], y: [0, Math.sin((i*30)*(Math.PI/180)) * 160], scale: [0, 1.5, 0], opacity: [0, 1, 0] }} transition={{ duration: 1.2, repeat: Infinity, repeatDelay: 1.5 }} />
                      ))}
                    </div>
                  )}
        ...
        <div className="w-full mt-4 md:mt-12 relative z-20 border-y border-blue-400/30">
          <a href="https://wa.me/917979826078?text=Hi" target="_blank" rel="noopener noreferrer" className="group block relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-500 transition-all py-3 md:py-4">
            <motion.div className="flex whitespace-nowrap text-white font-black text-xs md:text-lg tracking-widest uppercase items-center" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 35, ease: "linear", repeat: Infinity }}>
              {[...Array(8)].map((_, i) => (
                <span key={i} className="flex items-center">
                  ...
                  <span className="bg-white text-blue-700 px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-sm shadow-md flex items-center gap-1.5">Click Now <ArrowRight className="h-3 w-3 md:h-4 md:w-4" /></span>
      ...
      <section id="facilities" className="relative py-10 md:py-24 bg-slate-50 overflow-hidden border-t border-b border-slate-200 p-4">
        <div className="relative z-10 max-w-7xl mx-auto px-0 md:px-8">
          <motion.div initial={{ opacity: 0, y: 80 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.8 }} transition={{ duration: 1, ease: "easeOut" }} className="max-w-4xl mx-auto text-center mb-8 md:mb-16">
          ...
          <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 1.2 }} className="hidden md:grid w-full mx-auto max-w-6xl grid-cols-2 lg:grid-cols-3 gap-6">
            {labImages.map((img: string, idx: number) => (
              <div key={idx} className="relative aspect-[4/3] rounded-3xl overflow-hidden shadow-lg border border-slate-200 bg-white group">
                <img src={img} alt={formatImageName(img)} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent p-6 flex flex-col justify-end"><h4 className="text-white text-xl font-bold leading-tight drop-shadow-md">{formatImageName(img)}</h4></div>
              </div>
            ))}
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="md:hidden relative w-full aspect-[4/3] rounded-[2rem] overflow-hidden shadow-xl border border-slate-200 bg-slate-100">
            {isMounted && labImages?.length > 0 ? (
              <>
                <AnimatePresence mode="wait"><motion.img key={labIndex} src={labImages[labIndex]} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }} className="absolute inset-0 w-full h-full object-cover" /></AnimatePresence>
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent p-5 flex flex-col justify-end z-10"><h4 className="text-white text-lg font-bold drop-shadow-md pb-4">{formatImageName(labImages[labIndex])}</h4></div>
                <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5 z-20">
                   {labImages.map((_: any, i: number) => (<button key={i} onClick={() => setLabIndex(i)} className={`h-1.5 rounded-full transition-all duration-300 shadow-sm ${i === labIndex ? 'bg-blue-600 w-5' : 'bg-white/70 w-1.5'}`} />))}
                </div>
              </>
            ) : ( <div className="w-full h-full flex items-center justify-center text-slate-400"><ImageIcon className="h-10 w-10 opacity-50" /></div> )}
          </motion.div>
        </div>
      </section>
